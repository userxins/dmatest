...
int main() {
	...
	if (config::read())
		...
		std::cout << "[updater] Generating configuration file\n" <<std::endl;
	...
	if (g_game.buildNumber == updater::build_number) {
		...
		std::cout << "[cs2] Offsets may be out of date. Cheat may not fully function." << std::endl;
	}
...
#ifndef _UC
	std::cout << "\n[settings] In Game keybinds: [F5] enable/disable Extra Flags\n\t" << std::endl;
#else
	std::cout << "\n[settings] In Game keybinds: [F5] enable/disable Extra Flags\n\t" << std::endl;
#endif
	...
	while (GetMessage(&msg, NULL, 0, 0) && !finish)
	{
		...
		if (GetAsyncKeyState(VK_F5) & 0x8000) { config::show_extra_flags = !config::show_extra_flags; config::save(); Beep(700, 100); };
